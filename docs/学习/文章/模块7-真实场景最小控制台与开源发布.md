...
本文中的“真实场景”指真实可操作的产品使用流程，不代表模型已经接入真实供应商。默认演示使用离线 Mock Model 和 Mock Executor；真实模型的人工验证是单独的可选实验，文末会说明边界。
...
图 2：脱敏的本地 Mock 演示草稿确认页。用户可以在执行前查看事实、假设、任务依赖和校验结果。
...
图 3：脱敏的本地 Mock 演示 Run/Task 详情页。状态、任务和事件时间线由控制面返回，终态后停止轮询。
...
项目已经有 OpenAI-compatible HTTP Model Adapter，但默认关闭真实模型。适配器代码可以发送 `model`、`messages`、工具定义和 JSON Schema `response_format`，并处理工具调用后的下一轮消息；这只是协议实现，不代表已经验证过某个供应商。

在本机准备合法服务地址、模型名和个人 API Key 后，才可以进行人工验证。三项配置只写入 `.env.local`，不写入命令历史、截图或 Git：

```dotenv
WORKLOAD_MODEL_ENDPOINT=https://供应商提供的完整接口地址
...
1. 供应商是否接受工具定义和 JSON Schema；
2. 模型是否先调用只读目录工具，再生成合法 Draft JSON；
3. 草稿是否能通过服务端校验并产生合理的事实、假设和问题；
4. 失败、超时和限流是否映射为平台错误码；
5. 终端、日志和审计中是否没有 API Key；
6. 记录模型名、响应时间、Token 用量和费用时是否使用脱敏摘要。

该实验是低并发、人工触发的兼容性和质量检查，不放入 CI，也不能推导所有供应商都兼容。真实模型生成的内容还必须经过人工确认，不能绕过 Draft API 直接执行。
