...
代价是领取、心跳、完成和回收都会增加数据库负载；单 Run revision 还是串行提交点。如果模块 5 的多 Run 基准证明数据库已成为不可接受的瓶颈，再评审消息队列、Outbox 或状态分片，而不是现在提前增加两套事实来源。**Outbox（事务发件箱）** 是把待发布消息与业务状态写入同一个数据库事务，再由后台过程异步转发消息的模式，用于避免数据库与消息系统直接双写不一致。**状态分片（State Sharding）** 是按 Run 或其他稳定键把状态分散到多个独立存储分区，使不同分区可以并行处理；它会增加路由、跨分片查询和一致性复杂度。
...
完整环境、命令、原始结果和限制见模块 4 验证报告。数据库查询次数、连接池高水位、多 Run 总吞吐、持续 CPU/内存和队列深度需要模块 5 的指标与持续基准支持，当前不写无法稳定测量的数据。
...
- 指标、Trace、告警和故障注入平台尚未建立；
- Worker 只支持 Mock Executor；
- 没有真实资源限制、容器隔离和 Kubernetes 部署；
- Bearer Token 没有企业身份、证书和租户边界；
- 至少执行一次仍要求真实业务处理重复副作用。

因此下一步进入模块 5，不是继续堆叠 Worker 数量，而是建立可观测性和持续故障实验：测量队列深度、领取空转、数据库事务延迟、租约回收时间、单 Run 与多 Run 吞吐，定位 revision 串行点，并让日志、指标和 Trace 能从 Run 关联到 Worker 和 Attempt。**Trace（分布式调用链）** 记录一次请求或任务跨组件经过的步骤和耗时，用于定位延迟或失败发生在哪一段。只有先看见系统的真实行为，后续性能优化和模块 6 的受限执行环境才有可靠依据。
...
这套设计保证任务不会因 Worker 崩溃静默丢失，也不会让旧租约覆盖新结果，但不承诺只执行一次。真实副作用的幂等、单 Run 串行瓶颈、可观测性和受限执行环境仍需要后续模块继续解决。
