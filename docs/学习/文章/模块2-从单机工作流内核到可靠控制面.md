...
镜像本身只读，一个镜像可以创建多个彼此独立的容器。容器被删除不会自动删除镜像；数据卷也有独立生命周期，所以删除并重建 PostgreSQL 容器后，只要保留原数据卷，数据库文件仍可继续使用。安装 Docker、查看 Dashboard 和清理数据卷的操作步骤见[项目本地运行、演示与换电脑手册](../../部署/本地开发与配置.md)，本文只保留技术边界。
