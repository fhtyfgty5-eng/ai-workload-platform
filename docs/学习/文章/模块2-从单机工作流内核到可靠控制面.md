...
后续模块 3 已经在这套结构化提交流程上增加 Agent Runtime、模型与工具边界、自然语言工作流草稿和用户确认流程；模块 4 又把任务执行拆到独立 Worker，并用租约和心跳处理执行节点失联。后续进展不改变本文关于单实例控制面、当时故障实验和性能基线的证据边界。控制面高可用、生产容量和真实执行副作用仍然需要后续实验。
