...
## 4. 工作流草稿为什么不是普通 WorkflowDefinition
...
调用顺序是：查找工具、检查权限、建立单次工具超时 Context、执行工具、限制响应大小、记录审计事件。未知工具和缺少权限都返回 `tool_not_allowed`，并且不会进入工具实现。
...
```text
goal + definition + facts + assumptions + questions
     + validation + tool_calls + status
  -> canonical JSON
  -> SHA-256
  -> content_hash
```
...
## 11. 为什么选择 Mock Model 和 HTTP 适配器
...
### 11.3 为什么没有使用 LangChain 或 LangGraph
...
`confirm` 只输出定义，不创建 Workflow，也不启动 Run。之后仍要显式调用已有控制面命令，平台才会保存不可变版本。
