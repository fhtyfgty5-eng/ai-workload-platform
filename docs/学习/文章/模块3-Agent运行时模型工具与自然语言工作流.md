...
模块 4 计划把任务执行拆到独立 Worker：使用 PostgreSQL 持久化待分发事实，Worker 通过 HTTP 按空闲容量主动领取，用租约（带过期时间的任务执行权）和心跳（Worker 周期发送的存活与续约请求）判断临时执行权，再用隔离令牌（标识当前合法租约、用于拒绝旧写入的值）拒绝旧 Worker 的迟到结果。这里继续复用模块 2 的数据库和网络边界，是为了先验证多进程所有权，不同时引入消息队列、新的流式通信协议或 Kubernetes。代价是增加轮询请求、数据库分发压力和租约过期后的重复执行可能，因此模块 4 保持“任务不能静默丢失但可能重复”的至少执行一次语义，不承诺任务在任何故障下都只执行一次。
