...
这些限制决定了模块 3 的成果是“可靠 Agent 边界和自然语言草稿闭环”，不是完整商业 Agent 平台。后续模块 4 已经增加独立 Worker、租约和故障接管；可观测性、受限执行环境和最终控制台仍待后续建设，但都不能绕过本篇建立的工具权限、结构校验和人工确认规则。
...
没有先做真实文件、Shell 或容器工具，是因为任务副作用一旦真实发生，Worker 故障后的重复执行会更难判断和清理；没有先做 Kubernetes（部署和管理容器化应用的开源平台），是因为 Kubernetes 可以重启进程，却不会替平台判断某个 Attempt 的租约是否有效。先把执行所有权和迟到结果规则验证清楚，模块 5 才能系统观察这些故障，模块 6 才适合加入真实受限执行环境。
