...
模块 3 在已有的可靠工作流内核和控制面之上，增加了从自然语言目标到结构化工作流草稿的入口。本文先展开一次完整流转，再解释 Agent Runtime（管理模型调用、工具权限和运行边界的 Agent 运行时）、模型适配器、工具调用、权限、预算、草稿校验、内容哈希和人工确认如何协作。
...
- 当时没有多 Agent、多 Worker、容器任务环境或 Web 控制台；
- 工具白名单不是任意代码安全沙箱。

这些限制决定了模块 3 的成果是“可靠 Agent 边界和自然语言草稿闭环”，不是完整商业 Agent 平台。后续模块 4 已经增加独立 Worker、租约和故障接管，模块 5 已经增加日志、指标、Trace、告警和可重复故障实验；受限执行环境和最终控制台仍待后续建设，但都不能绕过本篇建立的工具权限、结构校验和人工确认规则。

### 为什么下一步进入模块 4

模块 3 解决的是“怎样把自然语言目标变成受控、可确认的结构化定义”，没有改变任务实际执行位置。确认后的 Run 仍由控制面进程中的 Mock Executor 推进，因此增加更多 Agent 功能也不能解决执行节点扩容、Worker 失联、任务重新分配和旧结果覆盖新结果的问题。

模块 4 随后把任务执行拆到独立 Worker：使用 PostgreSQL 持久化待分发事实，Worker 通过 HTTP 按空闲容量主动领取，用租约（带过期时间的任务执行权）和心跳（Worker 周期发送的存活与续约请求）判断临时执行权，再用隔离令牌（标识当前合法租约、用于拒绝旧写入的值）拒绝旧 Worker 的迟到结果。这里继续复用模块 2 的数据库和网络边界，是为了先验证多进程所有权，不同时引入消息队列、新的流式通信协议或 Kubernetes。实际验证也确认了对应代价：轮询请求、数据库分发压力和租约过期后的重复执行可能仍然存在，因此模块 4 保持“任务不能静默丢失但可能重复”的至少执行一次语义，不承诺任务在任何故障下都只执行一次。

没有先做真实文件、Shell 或容器工具，是因为任务副作用一旦真实发生，Worker 故障后的重复执行会更难判断和清理；没有先做 Kubernetes（部署和管理容器化应用的开源平台），是因为 Kubernetes 可以重启进程，却不会替平台判断某个 Attempt 的租约是否有效。先把执行所有权和迟到结果规则验证清楚，模块 5 才能系统观察这些故障；模块 5 已经完成这一步，模块 6 才适合加入真实受限执行环境。
