...
import type { EventPage, RunSummary, TaskPage, TaskDetail } from "../api/types";
...
export function TaskDetail({ client, runId, taskKey }: { client: ApiClient; runId: string; taskKey: string }): ReactElement {
  const [detail, setDetail] = useState<TaskDetail>();
  useEffect(() => { const controller = new AbortController(); void client.get<TaskDetail>(`/api/v1/runs/${encodeURIComponent(runId)}/tasks/${encodeURIComponent(taskKey)}`, controller.signal).then(setDetail); return () => controller.abort(); }, [client, runId, taskKey]);
  return <div>{detail?.task.attempts.map((attempt) => <div key={attempt.number}>Attempt {attempt.number}: <StatusBadge value={attempt.status} /></div>)}</div>;
}
