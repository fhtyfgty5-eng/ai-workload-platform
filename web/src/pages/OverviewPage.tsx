...
export function OverviewPage({ client }: { client: ApiClient }): ReactElement {
  ...
  return <section><header className="page-header"><p className="eyebrow">Overview</p><h1>平台概览</h1><p className="muted">从控制面返回的运行和执行节点摘要。</p></header><AsyncState loading={!runs && !workers} error={error}><div className="stat-grid">{["pending", "running", "succeeded", "failed"].map((status) => <div className="stat-panel" key={status}><span className="muted">{status}</span><strong>{counts[status] ?? 0}</strong></div>)}<div className="stat-panel"><span className="muted">在线 Worker</span><strong>{workers?.items.filter((worker) => worker.status === "active").length ?? 0}</strong></div></div><section className="content-panel"><h2>最近运行</h2>{runs?.items.length ? <div className="table-wrap"><table><thead><tr><th>Run</th><th>Workflow</th><th>状态</th><th>创建时间</th></tr></thead><tbody>{runs.items.map((run) => <tr key={run.run_id}><td><code>{run.run_id}</code></td><td>{run.workflow_id}</td><td><StatusBadge value={run.status} /></td><td>{new Date(run.created_at).toLocaleString()}</td></tr>)}</tbody></table></div> : <p className="muted">暂无运行记录</p>}</section><section className="content-panel"><h2>观测摘要</h2><pre className="metrics-preview">{metrics || "暂无 Metrics"}</pre></section></AsyncState></section>;
}
